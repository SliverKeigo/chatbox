...
export class OpenAIProvider extends BaseApiProvider {
  ...
  setProxyConfig(config: ProxyConfig | null): void {
    console.log('OpenAI: 设置代理配置:', config);
  ...
  setModelConfig(config: ModelConfig | null): void {
    ...
    console.log('OpenAI: 设置模型配置:', config);
  ...
  setCurrentModel(model: string): void {
    console.log('OpenAI: 设置当前模型:', model);
  ...
  private async streamMessageWithRetry(
    ...
    try {
      ...
      console.log(`OpenAI API请求诊断信息 (尝试 ${retryCount + 1}/${this.maxRetries + 1}):`, diagnosticInfo);
      ...
      try {
        ...
        console.log(`OpenAI 流式响应完成: 收到 ${chunkCount} 个数据块，总长度 ${fullResponse.length} 字符`);
        ...
        console.error(`OpenAI 流式操作失败 (尝试 ${retryCount + 1}/${this.maxRetries + 1}):`, streamError, diagnosticInfo);
        ...
        if (retryCount < this.maxRetries && 
          ...
          console.log(`OpenAI 流式请求失败，准备重试 (${retryCount + 1}/${this.maxRetries})...`);
      ...
      // 处理最终错误
      console.error('OpenAI API 流式请求最终失败:', error);
  ...
  async fetchAvailableModels(): Promise<ModelInfo[]> {
    try {
      ...
      console.log(`OpenAI: 找到 ${gptModels.length} 个可用模型`);
      return gptModels;
    } catch (error: any) {
